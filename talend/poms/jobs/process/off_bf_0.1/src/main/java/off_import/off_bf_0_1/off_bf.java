...
public class off_bf implements TalendJob {
	...
	private final String projectName = "OFF_IMPORT";
	...
	public static class rowsStruct implements
		...
		final static byte[] commonByteArrayLock_OFF_IMPORT_off_bf = new byte[0];
		static byte[] commonByteArray_OFF_IMPORT_off_bf = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_OFF_IMPORT_off_bf.length) {
					if (length < 1024
							&& commonByteArray_OFF_IMPORT_off_bf.length == 0) {
						commonByteArray_OFF_IMPORT_off_bf = new byte[1024];
					} else {
						commonByteArray_OFF_IMPORT_off_bf = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_OFF_IMPORT_off_bf, 0, length);
				strReturn = new String(commonByteArray_OFF_IMPORT_off_bf, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_OFF_IMPORT_off_bf) {
				...
				try {
					...
					this.ingredients_text = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.ingredients_text, dos);
		...
		public String toString() {
			...
			sb.append(",ingredients_text=" + ingredients_text);
			sb.append(",code=" + code);
			sb.append(",name=" + name);
	...
	public void tMongoDBInput_1Process(
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				pathMap_tMongoDBOutput_1.put("ingredients_text", "");

				pathMap_tMongoDBOutput_1.put("code", "");
				...
				pathMap_tMongoDBInput_1.put("ingredients_text", "");
				pathMap_tMongoDBInput_1.put("code", "");
				...
				while (cursor_tMongoDBInput_1.hasNext()) {
					...
					valueObj_tMongoDBInput_1 = dbObjectInputUtil_tMongoDBInput_1
							.getValue(pathMap_tMongoDBInput_1
									...
									"ingredients_text", o_tMongoDBInput_1);
					if (valueObj_tMongoDBInput_1 != null
							&& valueObj_tMongoDBInput_1.toString().length() > 0) {
						input.ingredients_text = valueObj_tMongoDBInput_1
								.toString();
					} else {
						input.ingredients_text = null;
					}
					valueObj_tMongoDBInput_1 = dbObjectInputUtil_tMongoDBInput_1
							.getValue(pathMap_tMongoDBInput_1.get("code"),
									"code", o_tMongoDBInput_1);
					if (valueObj_tMongoDBInput_1 != null
							&& valueObj_tMongoDBInput_1.toString().length() > 0) {
						input.code = valueObj_tMongoDBInput_1.toString();
					} else {
						input.code = null;
					}
					valueObj_tMongoDBInput_1 = dbObjectInputUtil_tMongoDBInput_1
					...
					if (ope_tFilterRow_1.getMatchFlag()) {
						...
						row1.ingredients_text = input.ingredients_text;
						row1.code = input.code;
					...
					if (row1 != null) {
						...
						{ // start of Var scope
							...
							rows_tmp.ingredients_text = row1.ingredients_text;
							rows_tmp.code = row1.code;
							...
							rows_tmp.quantity = row1.quantity;
							...
							try {
								...
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1
												.get("externId"), "externId",
										rows.externId);
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1
												.get("ingredients_text"),
										"ingredients_text",
										rows.ingredients_text);
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1.get("code"),
										"code", rows.code);
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1.get("name"),
										"name", rows.name);
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1
												.get("last_modified"),
										"last_modified", rows.last_modified);
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1
												.get("quantity"), "quantity",
										rows.quantity);
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1.get("stores"),
										"stores", rows.stores);
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1
												.get("countries_tags"),
										"countries_tags", rows.countries_tags);
								updateObjectUtil_tMongoDBOutput_1
										.put(pathMap_tMongoDBOutput_1
												.get("categories_tags"),
												"categories_tags",
												rows.categories_tags);
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1
												.get("categories_hierarchy"),
										"categories_hierarchy",
										rows.categories_hierarchy);
								updateObjectUtil_tMongoDBOutput_1.put(
										pathMap_tMongoDBOutput_1
												.get("nutriments"),
										"nutriments", rows.nutriments);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = off_bf.class.getClassLoader()
					.getResourceAsStream(
							"off_import/off_bf_0_1/contexts/" + contextStr
									+ ".properties");
			if (inContext == null) {
...
/************************************************************************************************
 * 82072 characters generated by Talend Open Studio for Big Data on the 3
 * octobre 2018 19:29:01 CEST
 ************************************************************************************************/
